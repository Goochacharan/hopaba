...
import { IndianRupee, Clock, MapPin, Phone, Instagram, Film, Tag } from 'lucide-react';
...
const BusinessCard: React.FC<BusinessCardProps> = ({ business, isShopPage = false }) => {
  ...
  const navigateToBusinessDetails = () => {
    // This is a placeholder - you'd need to create a business details page
    ...
    <Card key={business.id} className="overflow-hidden">
      <CardHeader className="bg-muted/30">
        ...
        <CardDescription className="line-clamp-none">
          <ScrollArea className="min-h-[8em] max-h-[8em] pr-3">
      ...
      <CardContent className="pt-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span>{business.availability || 'Availability not specified'}</span>
          </div>
          <div className="flex items-center gap-2">
            <MapPin className="h-4 w-4 text-muted-foreground" />
            <span>{business.area && business.city ? `${business.area}, ${business.city}` : 'Location not specified'}</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Phone className="h-4 w-4 text-muted-foreground" />
          <span>{business.contact_phone || 'Contact information not available'}</span>
        </div>
        <div className="flex items-center gap-2">
          <Instagram className="h-4 w-4 text-muted-foreground" />
          <span>{business.instagram || 'No Instagram profile'}</span>
          {business.instagram && (
            <button 
      ...
      <CardFooter className="border-t bg-muted/10 gap-2 justify-end">
        <Button 
          variant="default" 
          onClick={navigateToBusinessDetails}
