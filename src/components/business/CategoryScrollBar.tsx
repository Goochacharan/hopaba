...
const CategoryScrollBar: React.FC<CategoryScrollBarProps> = ({
  ...
  useEffect(() => {
    ...
    if (categoriesData && categoriesData.length > 0) {
      ...
      if (selected && selected !== "All") {
        ...
        const selectedCategory = categoriesData.find(cat => normalizeCategory(cat.name) === normalizedSelected);
  ...
  const fetchCategoriesFromServiceProviders = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('service_providers').select('category').eq('approval_status', 'approved').not('category', 'is', null);
      if (error) {
  ...
  return <div className="space-y-4">
      <div className={cn("w-full overflow-x-auto scrollbar-none flex gap-3 py-2 px-2", className)} style={{
      WebkitOverflowScrolling: "touch"
        ...
        <div className="flex gap-3 min-w-max">
          ...
          const normalizedSelected = normalizeCategory(selected);
          const isSelected = cat === "All" ? normalizedSelected === "all" || !selected : normalizedSelected === normalizedCat;
      ...
      {selected !== 'All' && !!currentCategoryId && onSubcategorySelect && <div className="px-2 animate-fadeIn">
          <div className="relative">
            <SubcategorySelector categoryId={currentCategoryId} value={selectedSubcategory} onChange={handleSubcategoryChange} className="w-full" />
            
            {selectedSubcategory && selectedSubcategory.length > 0 && <div className="mt-2 flex items-center justify-center">
